 ...
 */
@WebServlet("/user/offer_game")
public class OfferGameServlet extends HttpServlet implements Servlet {
	private static final long serialVersionUID = 2L;

	public OfferGameServlet() {
		super();
	}
		...
		if (user != null) {
			...
			if (library != null) {
				if (user.getLibrary().contains(library)) {
					...
					if (friend != null) {
						User friend_user = session.get(User.class, Integer.parseInt(friend));
						if (library.getActive() == 0 && user.getFriends().contains(friend_user)) {
							user.giveGame(friend_user, library, session);
						}
					}
				}
			}
